<?php
echo "Creating a test page to demonstrate the off-loading table functionality...\n\n";

// Test data based on the API response we just got
$testOffloadingPO = [
    'po_number' => 'PO-250906-024518',
    'dock_arrival_time' => '2025-09-05 17:17:12'
];
...
<html>
<head>
    <title>Test Off-loading Table</title>
...
<body class="bg-light">
    <div class="container mt-4">
        <h2>Off-loading Table Test</h2>
        
        <!-- Test the new Active Off-loading widget -->
        <div class="col-lg-4">
            <div class="bg-white rounded p-3 h-100 shadow">
        ...
        <div class="mt-4">
            <button id="testAddPO" class="btn btn-primary">Add Test PO</button>
            <button id="testClearPOs" class="btn btn-secondary">Clear All</button>
        </div>
    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            // Add the test PO automatically
            addToOffloadingTable("' . $testOffloadingPO['po_number'] . '", "' . $testOffloadingPO['dock_arrival_time'] . '");
            
            // Start the duration update interval
            setInterval(updateOffloadingDurations, 1000);
            
            $("#testAddPO").click(function() {
                addToOffloadingTable("PO-TEST-" + Date.now(), new Date().toISOString());
            });
            
            $("#testClearPOs").click(function() {
                ongoingOffloadingPOs = {};
                updateOffloadingTable();
            });
        });
    </script>
...
file_put_contents('test_offloading_table.html', $testHTML);
echo "✅ Created test_offloading_table.html\n";
echo "Open http://localhost/test_offloading_table.html to see the working table\n\n";

echo "The table should show:\n";
echo "- PO-250906-024518 with 17+ hours elapsed time\n";
echo "- Orange warning indicator (stuck status)\n";
echo "- Real-time duration updates\n";
echo "- Badge count showing 1 active off-loading\n";
?>